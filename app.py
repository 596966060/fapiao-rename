        ...
        """
        发票批量重命名工具 - 最终稳定版
        支持：普通发票（含住宿/打车）、火车票、飞机票、网约车（T3等）、合同（Word/PDF/图片）
        """
        ...
        class InvoiceExtractor:
            ...
            def _extract_name_from_section(self, text: str, section_label: str, name_label: str = '名称') -> str | None:
                pat1 = re.compile(
                    section_label + r'\s*[：:]?\s*[\n\r]+\s*' + name_label + r'\s*[：:]\s*([^\n\r]+)',
                    re.IGNORECASE
                )
                m = pat1.search(text)
                if m:
                ...
                pat2 = re.compile(
                    section_label + r'.*?' + name_label + r'\s*[：:]\s*([^\n\r]+)',
            ...
            def _extract_fields(self, text: str) -> dict:
                ...
                if not buyer_raw:
                    buyer_raw = self._extract_name_from_section(text, '购买方信息', '名称')
                if not buyer_raw:
                    buyer_raw = self._extract_name_from_section(text, '购买方', '名称')
                ...
                if not supplier_raw:
                    supplier_raw = self._extract_name_from_section(text, '销售方信息', '名称')
                if not supplier_raw:
                    supplier_raw = self._extract_name_from_section(text, '销售方', '名称')
        ...
        def detect_doc_type(text: str) -> str:
            ...
            if _TRAIN_KEYWORDS.search(text) or _TRAIN_NUMBER_RE.search(text):
                return 'train'
            if _CONTRACT_STRONG.search(text) or (_CONTRACT_PARTY_A.search(text) and _CONTRACT_PARTY_B.search(text)):
                return 'contract'
            return 'invoice'
        ...
        class TrainTicketExtractor:
            ...
            def _clean_station(self, name: str) -> str:
                ...
                if self._STATION_BLACKLIST.match(name):
                    return ''
                # 移除多余的后缀如“等级”“T3”等
                name = re.sub(r'\s*等级.*$', '', name).strip()
                name = re.sub(r'\s*T3\s*', '', name).strip()
            ...
            def _extract_fields(self, text: str, stem: str = '') -> dict:
                ...
                name_pats = [
                    r'(?:姓\s*名|旅\s*客|购\s*票\s*人|乘\s*客|乘客姓名|出行人)[：:\s]*([\u4e00-\u9fa5]{2,4})',
                ...
                # ==================== 核心：出发地 / 到达地提取（网约车专用） ====================
                # 1) 直接从“出发地”和“到达地”标签后提取，支持跨行
                def _extract_travel_stations(text):
                    # 匹配“出发地  香樟园-南2门  到达地  沈阳桃仙国际机场T3航站楼-国内出发”
                    patterns = [
                        r'出发地\s*([\u4e00-\u9fa5A-Za-z0-9\-（）()\s]{2,40}?)\s*到达地\s*([\u4e00-\u9fa5A-Za-z0-9\-（）()\s]{2,40}?)(?=\s*(?:等级|交通工具类型|\n|$))',
                        r'出发地\s*[:：]?\s*([^\n]{2,30})\s*到达地\s*[:：]?\s*([^\n]{2,30})',
                    ]
                    for pat in patterns:
                        ...
                        if m:
                            ...
                            t = m.group(2).strip()
                            # 去除可能混入的“等级”“交通工具类型”等
                            f = re.sub(r'\s*(?:等级|交通工具类型|无).*$', '', f).strip()
                            t = re.sub(r'\s*(?:等级|交通工具类型|无).*$', '', t).strip()
                            if f and t:
                                return f, t
                    return None, None

                travel_stations = _extract_travel_stations(text)
                if travel_stations[0] and travel_stations[1]:
                    result['from_station'] = _clean_st(travel_stations[0])
                    result['to_station'] = _clean_st(travel_stations[1])

                # 2) 如果未提取到，再用标准的标签匹配（兼容火车票）
                if not result['from_station'] or not result['to_station']:
                    ...
                    if _from_labeled and not result['from_station']:
                        result['from_station'] = _from_labeled
                    if _to_labeled and not result['to_station']:
                        result['to_station'] = _to_labeled
                ...
                if not result['from_station'] or not result['to_station']:
                    ...
                    if m:
                        if not result['from_station']:
                            result['from_station'] = _clean_st(m.group(1))
                        if not result['to_station']:
                            result['to_station'] = _clean_st(m.group(2))

                # 4) 保底占位
                if not result['from_station']:
                    result['from_station'] = '未知出发地'
                if not result['to_station']:
                    result['to_station'] = '未知到达地'
        ...
        def generate_travel_filename(data: dict, travel_type: str | None, original_ext: str) -> str:
            ...
            from_st = (data.get('from_station') or '未知出发地')[:15]
            to_st   = (data.get('to_station') or '未知到达地')[:15]
            price = data.get('price') or data.get('amount') or '0.00'
            parts = [date, from_st, to_st]
            if travel_type:
        ...
        def smart_extract(file_path: str, reader) -> tuple:
            ...
            if doc_type in ('train', 'flight', 'travel'):
                ...
                # 若为飞机票且未提取到出发/到达，尝试从文件名提取
                if doc_type == 'flight':
                    if not fields.get('from_station') or not fields.get('to_station'):
                        m = re.search(r'([\u4e00-\u9fa5]{2,8})[-—]([\u4e00-\u9fa5]{2,8})', fp.stem)
                        if m:
                            if not fields.get('from_station'):
                                fields['from_station'] = m.group(1)
                            if not fields.get('to_station'):
                                fields['to_station'] = m.group(2)
            elif doc_type == 'contract':
